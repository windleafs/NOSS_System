...
def encoderapi(img_path,model_path,output_path,user_id,filename,device='cuda'):
    img = Image.open(img_path).convert("RGB")
    device = get_device(device)
    encoder = model_import(model_path, model_name="Encoder", device=device)
    bch = BCHHelper()
    ...
    transforms.ToPILImage()(encoded_img).save(os.path.join(output_path, filename))
    #transforms.ToPILImage()(res_img + 0.5).save(os.path.join(output_path, 'residual.jpg'))
...
#     # os.chdir(sys.path[0])
#     # main(parse_args())
#     img_path="test/test_source/lena.jpg"
#     model_path="weight/latest-0.pth"
#     output_path="out/"
#     user_id=114223
#     encoderapi(img_path,model_path,output_path,user_id,'cpu')
